...
void runV1UpperControl(); void updateRawSpeedValueFromPulseInterval(); void updateDisplayFilter(); void updateTargetStepFromDisplay(); void updateVirtualNeedleControl(unsigned long nowMicros); void commandMotorPosition(float logicalStep);
...
/*
  Honda Beat Speedometer v2.1 - display filter 3:1 test
  ホンダビート スピードメーター v2.1 - 表示フィルタ3:1テスト

  Base / ベース:
    meter/speed/speed_v2.1/tests/speed_v2_1_analyzed_parameter_test/
    speed_v2_1_analyzed_parameter_test.ino

  Purpose / 目的:
    Evaluate a faster display filter response by changing only the weighting
    between the previous filtered value and the newest raw value from 7:1 to 3:1.
    解析・シミュレーション後に実車確認済みの最終テスト版をベースに、
    表示フィルタの旧値：新値の重みだけを7:1から3:1へ変更して評価します。

  Validated parameters retained / 継承する検証済みパラメータ:
    TARGET UP/DOWN       45/60 step/s
    VIRTUAL VEL UP/DOWN  500/500 step/s
    VIRTUAL ACC UP/DOWN  6000/6000 step/s^2
    STOP BAND            3 step
    MOTOR MAX SPEED      900 microstep/s
    MOTOR ACCEL          8000 microstep/s^2
  ...
  3:1 change / 3:1変更:
    ...
    The new raw value contribution changes from 1/8 (12.5%) to 1/4 (25%).
    新しい生値の寄与率を1/8 (12.5%)から1/4 (25%)へ増やします。

  Important / 重要:
    This 3:1 notation is a display-filter weighting ratio. It is NOT pulse
    division and NOT a mechanical or motor-to-needle reduction ratio.
    この3:1は表示フィルタの重み比です。パルス分周や機械減速比ではありません。
    Motor-position conversion, speed conversion, pulse handling and all other
    validated parameters remain unchanged from speed_v2_1_analyzed_parameter_test.
    モーター位置換算、速度換算、パルス処理、その他の検証済みパラメータは変更しません。
*/
...
const byte DISPLAY_FILTER_OLD=3,DISPLAY_FILTER_NEW=1;
...
const float TARGET_STEP_MAX_UP_PER_SEC=45.0f;
const float TARGET_STEP_MAX_DOWN_PER_SEC=60.0f;
...
unsigned long rawSpeedValue=0;float filteredDisplayValue=0.0f,rawTargetStepFromDisplay=0.0f,stabilizedTargetStep=0.0f,targetStepFromDisplay=0.0f;bool targetSlewInitialized=false;float virtualNeedleStep=0.0f,virtualNeedleVelocity=0.0f;int32_t lastCommandedMotorPosition=-1;unsigned long lastControlUpdateMicros=0;bool speedTimeoutZeroActive=false;
...
void runV1UpperControl(){updateRawSpeedValueFromPulseInterval();const unsigned long nowMicros=micros();if((unsigned long)(nowMicros-lastControlUpdateMicros)>=CONTROL_UPDATE_INTERVAL_US){updateDisplayFilter();updateTargetStepFromDisplay();updateVirtualNeedleControl(nowMicros);lastControlUpdateMicros=nowMicros;}}
void updateRawSpeedValueFromPulseInterval(){const unsigned long nowMicros=micros();unsigned long intervalCopy=0,lastSeenCopy=0;bool hasUpdatedInterval=false;noInterrupts();intervalCopy=latestPulseIntervalMicros;lastSeenCopy=lastPulseSeenMicros;hasUpdatedInterval=pulseIntervalUpdated;pulseIntervalUpdated=false;interrupts();unsigned long ageMicros=0;if(lastSeenCopy==0)ageMicros=0;else if(nowMicros>=lastSeenCopy)ageMicros=nowMicros-lastSeenCopy;else ageMicros=0;const bool timeoutNow=(lastSeenCopy==0||ageMicros>SPEED_PULSE_TIMEOUT_US);if(timeoutNow){rawSpeedValue=0;speedTimeoutZeroActive=true;return;}if(hasUpdatedInterval&&intervalCopy>=MIN_VALID_INTERVAL_US){const unsigned long value=SPEED_STEP_NUMERATOR/intervalCopy;rawSpeedValue=constrain(value,0UL,(unsigned long)(MOTOR_STEPS+SPEED_STEP_OFFSET));speedTimeoutZeroActive=false;}}
void updateDisplayFilter(){if(filteredDisplayValue<=0.0f){filteredDisplayValue=(float)rawSpeedValue;return;}filteredDisplayValue=(filteredDisplayValue*DISPLAY_FILTER_OLD+(float)rawSpeedValue*DISPLAY_FILTER_NEW)/(DISPLAY_FILTER_OLD+DISPLAY_FILTER_NEW);if(speedTimeoutZeroActive&&filteredDisplayValue<0.5f)filteredDisplayValue=0.0f;}
