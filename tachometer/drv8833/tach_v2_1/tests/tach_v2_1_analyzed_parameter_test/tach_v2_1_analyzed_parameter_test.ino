...
/*
  ...
  Purpose / 目的:
    Validate the latest parameter candidate derived from real-vehicle
    raw pulse logs and simulation while keeping the v2.1 control
    architecture unchanged.

    実車Rawパルスログとシミュレーションから導出した最新候補を、
    v2.1の制御構造を変更せずに実車確認するためのテストコードです。

  Parameter changes from v2.1 baseline / v2.1基準からの変更:
    CONTROL INTERVAL       100 ms -> 100 ms (unchanged / 変更なし)
    IIR                    7:1 per pulse -> 7:1 per pulse (unchanged / 変更なし)
    TARGET UP/DOWN         200/100 -> 425/160 logical step/s
    VIRTUAL VEL UP/DOWN    150/150 -> 425/160 logical step/s
    VIRTUAL ACC UP/DOWN    500/500 -> 2600/1500 logical step/s^2
    MOTOR MAX SPEED        3600 -> 4800 1/16 microstep/s
    MOTOR ACCEL            9600 -> 32000 1/16 microstep/s^2

  Notes / 注記:
    - This is a v2.1 parameter-variation test, not a new control version.
    - Motor speed/acceleration are deliberately increased to reduce the
      motor-layer lag observed in simulated rapid RPM rise cases.
    - Verify missed steps, resonance, abnormal vibration and zero return
      on the actual gauge before adopting these values in the baseline.

    - 本コードは新しい制御バージョンではなく、v2.1のパラメータ違いテストです。
    - 急な回転上昇時にシミュレーションで確認されたモーター層の遅れを低減するため、
      モーター最大速度・加速度を引き上げています。
    - 本体へ採用する前に、実機で脱調・共振・異常振動・ゼロ戻りを確認してください。
*/
...
private:
  ...
  void updateMotion(float maximumSpeed, float acceleration)
  {
    ...
    if (desiredDirection != 0 && velocityDirection != 0 &&
