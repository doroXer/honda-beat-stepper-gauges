...
/*
  Honda Beat Tachometer v2.1 / ホンダビート タコメーター v2.1
  ---------------------------------------------------------------

  1/16-microstep evolution of the v2.0 1/4-microstep baseline.
  v2.0の1/4マイクロステップ版を発展させた1/16マイクロステップ版です。

  Motor drive / モーター駆動:
    - X27.168 stepper motor
    - DRV8833 H-bridge driver
    - Dedicated 1/16 microstepping, 64 phase positions per electrical cycle
    - X27.168ステッピングモーター
    - DRV8833 Hブリッジドライバ
    - 1/16マイクロステップ専用、1電気周期64位相

  Upper control / 上位制御:
    The v2.0/v1.1 logical display-control architecture is retained.
    v2.0/v1.1由来の論理表示制御アーキテクチャを維持します。
  ...
  Notes / 注記:
    - The upper layer uses the logical-position scale 0..720 as float.
    - The motor boundary converts logical position to integer 1/16 microsteps.
    - 720 logical steps correspond to 7680 motor microsteps.
    - Software position is changed only by emitted physical microsteps.
    - When the target reverses relative to current motor velocity, velocity
      and residual step accumulation are cleared without changing position.

    - 上位層は0..720の論理位置をfloatで扱います。
    - モーター境界で整数の1/16マイクロステップ位置へ変換します。
    - 720 logical stepは7680 motor microstepに対応します。
    - ソフト上の現在位置は、実際に出力したmicrostepによってのみ更新します。
    - 目標方向が現在のモーター速度方向と反転した場合、位置は変更せず、
      速度と残留step accumulatorのみをクリアします。
*/
...
const float TARGET_STEP_MAX_UP_PER_SEC = 200.0f;
...
// ============================================================
// logical-step -> v2.1 1/16-microstep boundary conversion
// ============================================================
...
const int32_t MOTOR_MAX_SIXTEENTH_STEPS =
    ...
    MOTOR_POSITION_RATIO_DENOMINATOR;  // 720 -> 7680
...
// ============================================================
// DRV8833 / X27.168 1/16-microstep settings
// ============================================================
...
const int32_t ZERO_TRAVEL_SIXTEENTH_STEPS_ALIGNED =
    ((ZERO_TRAVEL_SIXTEENTH_STEPS + 63L) /
     64L) * 64L;

const unsigned long KEYOFF_ZERO_HOLD_MS = 200UL;

// ============================================================
// 1/16 microstep phase table
// ============================================================
...
const PhaseValue PHASE_TABLE[64] = {
  {   0,  255}, {  25,  254}, {  50,  250}, {  74,  244},
  {  98,  236}, { 120,  225}, { 142,  212}, { 162,  197},
  { 180,  180}, { 197,  162}, { 212,  142}, { 225,  120},
  { 236,   98}, { 244,   74}, { 250,   50}, { 254,   25},
  { 255,    0}, { 254,  -25}, { 250,  -50}, { 244,  -74},
  { 236,  -98}, { 225, -120}, { 212, -142}, { 197, -162},
  { 180, -180}, { 162, -197}, { 142, -212}, { 120, -225},
  {  98, -236}, {  74, -244}, {  50, -250}, {  25, -254},
  {   0, -255}, { -25, -254}, { -50, -250}, { -74, -244},
  { -98, -236}, {-120, -225}, {-142, -212}, {-162, -197},
  {-180, -180}, {-197, -162}, {-212, -142}, {-225, -120},
  {-236,  -98}, {-244,  -74}, {-250,  -50}, {-254,  -25},
...
// ============================================================
// Dedicated DRV8833 1/16-microstep motor layer
// ============================================================
...
public:
  ...
  {
    targetPosition_ = constrain(position, (int32_t)0, MOTOR_MAX_SIXTEENTH_STEPS);
  }
  ...
  void update()
  {
    updateMotion(MOTOR_MAX_SPEED_SIXTEENTH_STEP_PER_SEC,
                 MOTOR_ACCEL_SIXTEENTH_STEP_PER_SEC2);
  }
  ...
  void updateBlocking()
  {
    ...
    while (!isAtTarget()) {
      updateMotion(BLOCKING_MAX_SPEED_SIXTEENTH_STEP_PER_SEC,
                   BLOCKING_ACCEL_SIXTEENTH_STEP_PER_SEC2);
  ...
  void zero()
  {
    ...
    for (int32_t i = 0; i < ZERO_TRAVEL_SIXTEENTH_STEPS_ALIGNED; i++) {
      emitLogicalSixteenthStep(-1, ZERO_PWM_MAX);
  ...
  void stopMotion()
  {
    ...
    lastMotionUpdateUs_ = micros();
    wasMoving_ = false;
    if (enabled_) applyPhase(currentPhase_, HOLD_PWM_MAX);
  }
...
private:
  ...
  void updateMotion(float maximumSpeed, float acceleration)
  {
    ...
    if (elapsedUs == 0UL) return;
    lastMotionUpdateUs_ = nowUs;

    float dtSec = (float)elapsedUs / 1000000.0f;
    if (dtSec > MOTOR_MAX_UPDATE_DT_SEC) dtSec = MOTOR_MAX_UPDATE_DT_SEC;
    ...
    if (error == 0) {
      motorVelocity_ = 0.0f;
      stepAccumulator_ = 0.0f;
      if (wasMoving_) applyPhase(currentPhase_, HOLD_PWM_MAX);
      wasMoving_ = false;
      return;
    }
    ...
    const int8_t velocityDirection =
        motorVelocity_ > 0.0f ? 1 : motorVelocity_ < 0.0f ? -1 : 0;
    ...
    const float stoppingDistance = (speedAbs * speedAbs) / (2.0f * acceleration);
    float accelerationCommand = 0.0f;

    if (velocityDirection != 0 && velocityDirection != desiredDirection) {
      accelerationCommand = -(float)velocityDirection * acceleration;
    }
    else if ((float)labs(error) <= stoppingDistance + 1.0f) {
      accelerationCommand = -(float)velocityDirection * acceleration;
    }
    else {
      accelerationCommand = (float)desiredDirection * acceleration;
    }

    motorVelocity_ += accelerationCommand * dtSec;
    motorVelocity_ = constrainFloat(motorVelocity_, -maximumSpeed, maximumSpeed);

    if (desiredDirection > 0 && motorVelocity_ < 0.0f) motorVelocity_ = 0.0f;
    if (desiredDirection < 0 && motorVelocity_ > 0.0f) motorVelocity_ = 0.0f;
    ...
    while (fabsf(stepAccumulator_) >= 1.0f &&
      ...
      const int8_t stepDirection = stepAccumulator_ > 0.0f ? 1 : -1;

      emitLogicalSixteenthStep(stepDirection, RUN_PWM_MAX);
    ...
    if (currentPosition_ == targetPosition_ && fabsf(stepAccumulator_) < 1.0f) {
      motorVelocity_ = 0.0f;
      stepAccumulator_ = 0.0f;
      applyPhase(currentPhase_, HOLD_PWM_MAX);
      wasMoving_ = false;
  ...
  void emitLogicalSixteenthStep(int8_t logicalDirection, uint8_t pwmMax)
  {
    const int8_t physicalDirection = logicalDirection * SCALE_DIRECTION;
    currentPhase_ += physicalDirection;
    while (currentPhase_ < 0) currentPhase_ += 64;
    while (currentPhase_ >= 64) currentPhase_ -= 64;
  ...
  void applySignedPhase(uint8_t pinPositive,
                        uint8_t pinNegative,
...
void beginKeyOffReturn()
{
  const float currentLogicalPosition =
      motorPositionToLogicalStep(tachMotor.getCurrentPosition());
...
void onTachPulse()
{
  const unsigned long nowUs = micros();
  if (lastPulseUs != 0UL) {
    latestPulseIntervalUs = nowUs - lastPulseUs;
...
void runV1UpperControl()
{
  unsigned long pulseIntervalUs = 0UL;
  bool pulseAvailable = false;
  unsigned long pulseTimestampUs = 0UL;

  noInterrupts();
  if (newPulseAvailable) {
    pulseIntervalUs = latestPulseIntervalUs;
  ...
  if (pulseAvailable && isValidTachInterval(pulseIntervalUs)) {
    lastAcceptedPulseUs = pulseTimestampUs;
    updateRawValueFromPulseInterval(pulseIntervalUs);
    const bool wasConfirmed = engineRunningConfirmed;

    if (updateEngineRunConfirmation(pulseIntervalUs)) {
      if (!wasConfirmed) initializeDisplayFilterFromRaw();
      updateDisplayFilter();
...
void updateTargetStepFromDisplay()
{
  long stepValue = filteredDisplayValue - STEP_OFFSET;
  if (stepValue < 0L) stepValue = 0L;
  if (stepValue > MOTOR_STEPS) stepValue = MOTOR_STEPS;
  rawTargetStepFromDisplay = (float)stepValue;
}
...
void updateVirtualNeedleControl(unsigned long nowMicros)
{
  ...
  const unsigned long elapsedUs = nowMicros - lastControlUpdateUs;
  ...
  const float targetDifference = rawTargetStepFromDisplay - stabilizedTargetStep;
  float maximumTargetChange =
      (targetDifference > 0.0f ? TARGET_STEP_MAX_UP_PER_SEC : TARGET_STEP_MAX_DOWN_PER_SEC) * dtSec;

  if (fabsf(targetDifference) <= maximumTargetChange) {
    stabilizedTargetStep = rawTargetStepFromDisplay;
  }
  else {
    stabilizedTargetStep += targetDifference > 0.0f ? maximumTargetChange : -maximumTargetChange;
  }
  ...
  const float positionError = targetStepFromDisplay - virtualNeedleStep;

  if (fabsf(positionError) <= VIRTUAL_STOP_BAND_STEP) {
  ...
  const float desiredVelocity = constrainFloat(
      positionError / dtSec,
      -VIRTUAL_MAX_VEL_DOWN_STEP_PER_SEC,
       VIRTUAL_MAX_VEL_UP_STEP_PER_SEC);

  if (desiredVelocity > virtualNeedleVelocity) {
    virtualNeedleVelocity += VIRTUAL_ACCEL_UP_STEP_PER_SEC2 * dtSec;
    if (virtualNeedleVelocity > desiredVelocity) virtualNeedleVelocity = desiredVelocity;
  }
  else if (desiredVelocity < virtualNeedleVelocity) {
    virtualNeedleVelocity -= VIRTUAL_ACCEL_DOWN_STEP_PER_SEC2 * dtSec;
    if (virtualNeedleVelocity < desiredVelocity) virtualNeedleVelocity = desiredVelocity;
  }

  virtualNeedleVelocity = constrainFloat(
  ...
  if (positionError > 0.0f && virtualNeedleStep > targetStepFromDisplay) {
    virtualNeedleStep = targetStepFromDisplay;
    virtualNeedleVelocity = 0.0f;
  }
  if (positionError < 0.0f && virtualNeedleStep < targetStepFromDisplay) {
  ...
  virtualNeedleStep = constrainFloat(virtualNeedleStep, 0.0f, (float)MOTOR_STEPS);
  commandMotorPosition(virtualNeedleStep);
}
...
void handleEngineStopZeroReturn()
{
  if (lastAcceptedPulseUs == 0UL) return;
  const unsigned long nowUs = micros();
  if ((unsigned long)(nowUs - lastAcceptedPulseUs) < NO_PULSE_STOP_TIMEOUT_US) return;
