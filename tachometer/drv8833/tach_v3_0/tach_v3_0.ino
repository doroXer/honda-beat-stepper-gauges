...
/*
  ...
  Main changes from maintained v2.1 tuned / v2.1 Tunedからの主変更:
    Display filter        7:1 pulse IIR -> 3-pulse moving average
    Control interval      100 ms -> 50 ms
    TARGET UP/DOWN        425/160 -> 1600/1600 logical step/s
    VIRTUAL VEL UP/DOWN   425/160 logical step/s (retained)
    VIRTUAL ACC UP/DOWN   2600/1500 logical step/s^2 (retained)
    STOP BAND             5 -> 0 logical step
    MOTOR MAX SPEED       4800 motor-position step/s (retained)
    MOTOR ACCEL           32000 -> 12000 motor-position step/s^2

  Notes / 注記:
    - The 3-pulse moving average is updated on each accepted tach pulse.
    - rawTarget is refreshed per accepted pulse from the latest average.
    - stabilized TARGET and Virtual motion are updated at 50 ms intervals.
    - STOP BAND processing remains in code but is disabled with value 0.
    - Motor acceleration 12000 corresponds to 1125 logical step/s^2 because
      logical-to-motor position conversion is 32/3.
    - Missing-pulse integer-multiple correction is intentionally NOT included.
*/
...
const byte DISPLAY_MOVING_AVERAGE_SAMPLES = 3;
...
const float MOTOR_ACCEL_SIXTEENTH_STEP_PER_SEC2 = 12000.0f;
...
long rawTachValue=0L,filteredDisplayValue=0L;
long displayMovingAverageBuffer[DISPLAY_MOVING_AVERAGE_SAMPLES]={0L,0L,0L};
long displayMovingAverageSum=0L;
byte displayMovingAverageIndex=0;
float rawTargetStepFromDisplay=0.0f,stabilizedTargetStep=0.0f,targetStepFromDisplay=0.0f,virtualNeedleStep=0.0f,virtualNeedleVelocity=0.0f;
unsigned long lastControlUpdateUs=0UL;
byte engineRunConfirmCount=0;
bool engineRunningConfirmed=false,displayFilterInitialized=false;
...
void resetDisplayStateAtPosition(float logicalPosition){ logicalPosition=constrainFloat(logicalPosition,0.0f,(float)MOTOR_STEPS); rawTachValue=0L; filteredDisplayValue=0L; clearMovingAverageFilter(); rawTargetStepFromDisplay=logicalPosition; stabilizedTargetStep=logicalPosition; targetStepFromDisplay=logicalPosition; virtualNeedleStep=logicalPosition; virtualNeedleVelocity=0.0f; lastControlUpdateUs=micros(); engineRunConfirmCount=0; engineRunningConfirmed=false; displayFilterInitialized=false; noInterrupts(); latestPulseIntervalUs=0UL; lastPulseUs=0UL; newPulseAvailable=false; interrupts(); lastAcceptedPulseUs=0UL; commandMotorPosition(logicalPosition); }
...
void clearMovingAverageFilter(){ for(byte i=0;i<DISPLAY_MOVING_AVERAGE_SAMPLES;i++) displayMovingAverageBuffer[i]=0L; displayMovingAverageSum=0L; displayMovingAverageIndex=0; }
void initializeDisplayFilterFromRaw(){ clearMovingAverageFilter(); for(byte i=0;i<DISPLAY_MOVING_AVERAGE_SAMPLES;i++){ displayMovingAverageBuffer[i]=rawTachValue; displayMovingAverageSum+=rawTachValue; } filteredDisplayValue=rawTachValue; displayFilterInitialized=true; }
void updateDisplayFilter(){ if(!displayFilterInitialized){ initializeDisplayFilterFromRaw(); return; } displayMovingAverageSum-=displayMovingAverageBuffer[displayMovingAverageIndex]; displayMovingAverageBuffer[displayMovingAverageIndex]=rawTachValue; displayMovingAverageSum+=rawTachValue; displayMovingAverageIndex++; if(displayMovingAverageIndex>=DISPLAY_MOVING_AVERAGE_SAMPLES) displayMovingAverageIndex=0; filteredDisplayValue=displayMovingAverageSum/(long)DISPLAY_MOVING_AVERAGE_SAMPLES; }
